...
func NewRouter(cfg *config.Config, stores *store.Stores, oauth *auth.DiscordProvider, hub *ws.Hub, bot *discord.Bot) http.Handler {
	...
	r.Route("/api", func(r chi.Router) {
		...
		r.Group(func(r chi.Router) {
			...
			r.Get("/wardens/export", s.handleExportWardens)
			r.Post("/wardens/import", s.handleImportWardens)
			...
			r.Put("/groups/{groupID}/access", s.handleSetAccessMode)
			...
			r.Route("/admin", func(r chi.Router) {
				...
				r.Post("/users/{userID}/promote", s.handleAdminPromote)
				r.Post("/users/{userID}/demote", s.handleAdminDemote)
				r.Patch("/users/{userID}", s.handleAdminSetCharacterName)
			})
		})
	})
