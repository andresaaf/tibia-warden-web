...
// form: rarity → difficulty → gold amount ("30k", "1.5kk"; blank = free for
// that combination). At least one price must be set.
func parseAttendPricing(prices map[string]map[string]string) (map[string]map[string]int64, error) {
	out := map[string]map[string]int64{}
	total := 0
	for _, rarity := range []string{models.RarityCommon, models.RarityUncommon} {
		byDifficulty := map[string]int64{}
		for key, raw := range prices[rarity] {
			if !slices.Contains(models.Difficulties, key) {
				return nil, fmt.Errorf("unknown difficulty %q", key)
			}
			...
			if err != nil {
				return nil, fmt.Errorf("%s %s: enter a price like 30k", rarity, key)
			}
			if v > 0 {
				byDifficulty[key] = v
				total++
		...
		out[rarity] = byDifficulty
	}
	for rarity := range prices {
		if rarity != models.RarityCommon && rarity != models.RarityUncommon {
			return nil, fmt.Errorf("unknown rarity %q", rarity)
		}
	}
	if total == 0 {
		return nil, errors.New("set at least one price")
	}
	return out, nil
}
...
// the submitted prices per difficulty and rarity.
func (s *Server) handleSetAccessMode(w http.ResponseWriter, r *http.Request) {
	...
	case models.AccessPayToAttend:
		prices, err = parseAttendPricing(body.Prices)
		if err != nil {
	...
	if err := s.stores.Groups.SetAccessMode(r.Context(), groupID, body.Mode, prices); err != nil {
